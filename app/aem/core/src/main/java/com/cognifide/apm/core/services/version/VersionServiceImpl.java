...
@Component(property = {Property.DESCRIPTION + "APM Version Service", Property.VENDOR})
public class VersionServiceImpl implements VersionService {
  ...
  public ScriptVersion getScriptVersion(ResourceResolver resolver, Script script) {
    String scriptVersionPath = getVersionPath(script);
    return Optional.ofNullable(resolver.getResource(scriptVersionPath))
        .map(resource -> resource.adaptTo(ScriptVersion.class))
        .orElse(new ScriptVersionModel(script.getPath()));
  }

  @Override
  public String getVersionPath(Script script) {
    return String.join("/", ImmutableList.of(VERSIONS_ROOT, normalizedPath(script), script.getChecksum(), SCRIPT_NODE_NAME));
  }


  @Override
  public String countChecksum(Iterable<Script> root) {
    String checksums = StreamSupport.stream(root.spliterator(), false).map(Script::getData).map(DigestUtils::md5Hex).collect(Collectors.joining());
    return DigestUtils.md5Hex(checksums);
  }

  @Override
  public void updateVersionIfNeeded(ResourceResolver resolver, Script script) {
    try {
      ...
      if (StringUtils.equals(checksum, script.getChecksum())) {
        new MutableScriptWrapper(script).setChecksum(checksum);
      }
  ...
  private String getScriptVersionPath(Script script) {
    return String.join("/", ImmutableList.of(VERSIONS_ROOT, normalizedPath(script)));
  }
  ...
  private String normalizedPath(Script script) {
    return script.getPath().replace("/", "_").substring(1);
  }
}
