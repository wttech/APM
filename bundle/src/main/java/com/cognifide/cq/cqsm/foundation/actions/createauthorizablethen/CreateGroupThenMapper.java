...
public class CreateGroupThenMapper extends BasicActionMapper {

    public static final String REFERENCE = "Create a group then set this group as a current authorizable for execution context.";

    @Mapping(
            value = {"CREATE" + DASH + "GROUP" + SPACE + STRING + SPACE + "THEN"},
            args = {"groupId"},
            reference = REFERENCE
    )
    ...
    @Mapping(
            value = {"CREATE" + DASH + "GROUP" + SPACE + STRING + SPACE + PATH + SPACE + "THEN"},
            args = {"groupId", "path"},
            reference = REFERENCE
    )
    ...
    @Mapping(
            value = {"CREATE" + DASH + "GROUP" + SPACE + STRING + SPACE + ("IF" + DASH + "NOT" + DASH + "EXISTS") + SPACE + "THEN"},
            args = {"groupId"},
            reference = REFERENCE
    )
    ...
    @Mapping(
            value = {"CREATE" + DASH + "GROUP" + SPACE + STRING + SPACE + PATH + SPACE + ("IF" + DASH + "NOT" + DASH + "EXISTS") + SPACE + "THEN"},
            args = {"groupId", "path"},
            reference = REFERENCE
    )
    ...
    private Action mapAction(String id, String path, Boolean ifNotExists) {
        return new CreateAuthorizableThen(id, null, path, ifNotExists, CreateAuthorizableStrategy.GROUP, true);
    }
